...
#define MAX_TOKEN_STRING_LENGTH     200 //MAX_TOKEN_STRING_LENGTH = SIMBOLOS MAXIMOS PERMITIDOS
#define MAX_BUFF                    200 //MAX_BUFF = MAXIMA CANTIDAD DE CARACTERES PERMITIDOS DE ENTRADA
#define MAX_CODE					200 //
#define MAX_OUTPUT_BUFF	            200

#include <ctype.h> /** añadido para quitar warnings, previa declaracion implicita **/
#include <stdio.h>
...
void factor() {
	...
	//char dummy,dummy2;
	if ( atom( ch ) ) {
		/*meter en pila de atomicas usadas a ch
		 NO MODIFIQUEN ESTO PARTA LAS PRUBEAS A NO SER QUE SEA NECESARIO METERLO A */
		used = used | (1 << ch - 'a');
		gen( ch );
		get_token();
	}
    ...
    /*    dummy = ch;
        dummy2 = ch-1;
        if ( dummy == dummy2) error("Error! Doble Operando Ingresado Sin Operador Previo ");
    else*/
		switch ( ch ) {
			case '-':
...
/*----------------------------------------------------------------------------*/
/* Logica operacional                                                         */
/*  Recursividad y defincion de simbolos                                      */
/*----------------------------------------------------------------------------*/
void term() {
...
/*----------------------------------------------------------------------------*/
/* Funciones auxiliares(codigo robusto?)                                      */
/*----------------------------------------------------------------------------*/
BOOL atom( char c ) {
...
/*----------------------------------------------------------------------------*/
/* Impresion de sintesis(sinopsis?) de proposicion                            */
/*----------------------------------------------------------------------------*/
...
/* Error handler                                                              */
/*----------------------------------------------------------------------------*/
//Formato de immpresion en tabla, burdo pero eficiente
...
void init() {
	...
	used = used | ( 1 << ( 'z' - 'a' + 1 ) );		// Añadir sentinela
...
/*	min = debug = showcode = true;
	int i;	// parse commands
	for ( i = 0; i < argc; i++ ) {
		switch( toupper( argv[i][0] ) ) {
			case 'M': min      = false; break;
	 		case 'D': debug    = true; break;
			case 'C': showcode = false; break;
		}
	}*/
	setjmp( start );
	do {
        printf("Ingrese expresion a evaluar usando 'a..z'\n TABLA DE SIMBOLOS:\n'&' -> conjuncion \n'v'  -> disyuncion \n'-' -> negacion \n'>' -> implicacion\n'=' -> bicondicional\n Finalice expresion con un '.'\n ");
		init();
		get_token();
		formula();
